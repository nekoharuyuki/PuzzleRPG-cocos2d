...
var ResultLayer = cc.Layer.extend({
    ...
    ctor:function () {
        ...
        this.dropCoin = parseInt( MapData.map[playerValue.mapid].dropCoin * (0.8 + rnd) );;
        ...
        for(var i = 0; i < range; i++ ){
            ...
            cc.log( "charId = " + charId );
            var tmpchar = new charIconSprite( charId );
            iconArray[i].addChild( tmpchar );
            CharStorage.push( getChar( charId, 1, -1 ) );
        }
        playBgm( 2 );
        if( playerValue.field <= playerValue.mapid ){
            playerValue.field++;
            if(playerValue.field >= 30 ){
                playerValue.field = 29;
            }
        }
        this.scheduleUpdate();
        dataSave();
