...
//		cc.log("mouse down "+ target.checkJewelryWaite);
		if(!target.myturn){
			return false;
		}
...
//				cc.log("mouse down Contains "+ x +":" + y + ":"+target.jewelrySpriteArray[x][y]);
				var jewelry = target.jewelrySpriteArray[x][y];
				
				if (jewelry == null){
//					cc.log("mouse down "+ jewelry);
					continue;
				}
				if(jewelry.state != JSTATUS["STOP"]){
//					cc.log("mouse down state "+ jewelry.state);
					continue;
				}
				
				var rect = cc.rect(jewelry.x - (target.jewelrySize / 2), jewelry.y - (target.jewelrySize / 2), jewelry.width, jewelry.height) ;
//				cc.log("mouse down Contains "+ x +":" + y );
				if( cc.rectContainsPoint(rect, location) ){
//					cc.log("mouse down select "+ jewelry);
			...
			if(target.moveCnt == 0){//わざと移動制限を入れる
				...
				for (var y = 0; y < target.rowNum; y++) {
					for (var x = 0; x < target.columNum; x++) {
						var jewelry = target.jewelrySpriteArray[x][y];
						if (jewelry == null){ 
//							cc.log("mouse move jewelry null");
							continue;
						}
						if (jewelry == target.selectJewelry) {
//							cc.log("mouse move jewelry = select");
							continue;
						}
						if(jewelry.state != JSTATUS["STOP"]){
//							cc.log("mouse move jewelry state ");
							continue;
						}
						var rect = cc.rect(jewelry.x - (target.jewelrySize / 2), jewelry.y - (target.jewelrySize / 2), jewelry.width, jewelry.height) ;
						if( cc.rectContainsPoint(rect, location) ){
...
//		cc.log("mouse up" + target.moveCnt);
		target.checkJewelry();
	},
